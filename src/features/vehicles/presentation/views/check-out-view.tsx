...
export default function CheckOutView({ reservationId }: CheckOutViewProps) {
  const { checkoutReservation, loading } = useReservationOperations();
  ...
  if (reservation.status !== ReservationStatus.Confirmed) {
    return <InfoError text="La reserva ya ha sido entregada" />;
  }
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Check-out del Vehículo</CardTitle>
            <CardDescription>
              Registre la entrega del vehículo al cliente
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter>
            <Button
              ...
              className="w-full"
              onClick={() =>
                checkoutReservation(reservation.id, parseInt(mileage))
              }
            >
              Check-out{" "}
              {loading ? (
